...
- How do we determine where is the rock that blocks it?
...
direction = ">>"
for y in N to 1:
    next = look up index of the rock with height y
    if direction == ">>":
...
```
- How do we lookup the index of the rock with height y in O(1) time? You need to do that in constant time in order to have an O(N) algorithm.
